...
array<action, 4> chooseActions()
{
	...
	action slash = { "Slashing Strike", 60, 20, "Does more damage but has less accuracy to hit compared to lunge" };
	action lunge = { "Lunging Strike", 80, 15, "Does less damage bus has more accuracy compared to slash" };
	action parry = { "Swift Parry", 100, 1, "Chance to negate an attacks over the next few turn"};
	action block = { "Stallwart Block", 100, 1, "Reduces the damage in half" };
	action heal = { "Spell of Healing Surge", 100, 20, "Recovers from 6 - 20 hp" };
	...
	//Displays list.
	cout << "\nWhich four actions will your hero take?\n";
...
void validateAttack(action attack, character& attacker, character& target)
{
	...
	else
	{
		...
		else if (strcmp(attack.name, "Spell of Searing Fire") == 0)
		{
			...
			if (mana > 0)
			{
				int health = rand() % attack.strength + mana * 3 - (target.defense / 2);
